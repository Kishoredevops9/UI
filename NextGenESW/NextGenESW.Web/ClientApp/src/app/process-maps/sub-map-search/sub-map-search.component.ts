...
@Component({
  selector: 'app-sub-map-search',
  templateUrl: './sub-map-search.component.html',
  styleUrls: ['./sub-map-search.component.scss']
})
export class SubMapSearchComponent implements OnInit,AfterViewInit {

  contentType='M';
  searchsubMapList;
  taskSubMapDataSource=[];
  ...
  constructor(private activityService: ProcessMapsService, 
              ...
              private taskSMservice: TaskCrationPageService,
              ) {
  ...
  loadServiceData(){ 
    if(this.contentType=='M'){ 
      ...
      const searchAssetType = 'M';
      this.taskSMservice.getSearchPopupData(searchAssetType).subscribe(
        (data) => {
          console.log(data);
            var res = JSON.parse(JSON.stringify(data));
            this.searchsubMapList = res.hits.hits;
            console.log(this.searchsubMapList);
            this.searchsubMapList.forEach((el) => {
              this.taskSubMapDataSource.push(el._source);
            });
            console.log(this.taskSubMapDataSource);

          this.dataLists = this.taskSubMapDataSource;  
          this.dataSourceActivitySearch = new MatTableDataSource(this.taskSubMapDataSource);
          this.originalData = this.dataSourceActivitySearch.data;
          this.dataSourceActivitySearch.paginator = this.paginations;
          //this.rowCount();
          this.dataSourceActivitySearch.sort = this.sorting;
