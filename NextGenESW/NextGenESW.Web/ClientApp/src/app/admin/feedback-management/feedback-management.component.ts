...
export class FeedbackManagementComponent implements OnInit {
  ...
  loadFeedbackData(){
    ...
    this.adminService.getAllEKSFeedbacks().subscribe( (data) => {
      ...
      if(this.selectedDropdownValues){
        displayFilter = this.selectedDropdownValues;
      }else if(this.selectDropdownData[0]){
        displayFilter = this.selectDropdownData[0];
      }else{
        displayFilter = this.selectType;
      }

      console.log("apiData", apiData);

      Object.keys(apiData).forEach(key => {
        if (apiData[key].isActive === true && apiData[key].title === displayFilter) {
      ...
      this.dataSourceFeedback = new MatTableDataSource(pushData);
  ...
  onFeedbackDelete(element){
    this.adminService.removeEKSFeedbacks(element.id).subscribe( (data) => {
      if(data){
        this.loadFeedbackData();
      }
    });
      ...
      helpData.forEach(element => {
        if(element.children){
          element.children.forEach(elem => {
            this.selectDropdownData.push(elem.title);
          });
        }else{
          this.selectDropdownData.push(element.title);
        }
      });
    });
    
    this.adminService.getAllUserGuides().subscribe( (data) => {
      userguideData = data;
      userguideData.forEach(element => {
        if(element.children){
          element.children.forEach(elem => {
            this.selectDropdownData.push(elem.title);
          });
  ...
  onSelectChange(selectedData){
    this.selectedDropdownValues = selectedData;
    this.loadFeedbackData();
  }
