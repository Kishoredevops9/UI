...
@Component({
  selector: 'app-eks-filter-section',
  templateUrl: './eks-filter-section.component.html',
  styleUrls: ['./eks-filter-section.component.scss'],
})
export class EksFilterSectionComponent implements OnInit {
  count: number;
  collection: any;
  EKSCollection: any;
  ...
  @Input() eksPopupSearchData:any;
  @Input() eksPopupPhaseData:any;
  ...
  ngOnChanges() {
    console.log("eksPopupPhaseData::ngOnChanges", this.eksPopupPhaseData);
    console.log("eksSearchOutput::ngOnChanges", this.eksPopupSearchData);
    let allEKSCollection_data = this.allEKSCollection ? this.allEKSCollection : [];
    let eksPopupSearchData_data = this.eksPopupSearchData ? this.eksPopupSearchData : [];
    allEKSCollection_data.map((node)=>{
      node['active']= false;
    });
    eksPopupSearchData_data.forEach(element => {
      allEKSCollection_data.map((node)=>{
        if (node.keyword==element.assettypecode ){
            node['active']= true;
        }
      })
    });

    this.loadFilterPhaseData();
  ...
  ngOnInit(): void {
    ...
    if (this.stepPopUp == 'AP_SF_SP_Not') {
      let content;
  ...
  onChangeEKSCollection(contentType) {
    ...
    if (this.checked && this.checked.length) {
      this.eksSearchCount.emit(this.checked.length);
      console.log('xxx this.checked.length', this.checked.length);
      ...
      console.log('xxx this.checked.length', 0);
    }
  ...
  loadFilterPhaseData() {
    console.log("loadFilterPhaseData", this.eksPopupPhaseData);
      this.filterData = [];
      this.filterData = this.eksPopupPhaseData;
      let tempObj = [];
      ...
      console.log("loadFilterPhaseData", this.filterData);
  ...
  ngOnDestroy() {
    //console.log('inside filter this.allEKSCollection',this.allEKSCollection);
    //this.searchTerm = '';
    // this.checked = this.allEKSCollection.filter((item) =>
    // item.checked = false
    // );
    this.checked = this.allEKSCollection.forEach((item) => item.checked = false
    );
    console.log('inside filter this.allEKSCollection after2',this.allEKSCollection);
    //this.searchTerm = '';
  }
}
