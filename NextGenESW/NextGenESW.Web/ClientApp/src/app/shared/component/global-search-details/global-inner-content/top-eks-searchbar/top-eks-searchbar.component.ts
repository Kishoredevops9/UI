...
import { Router, Event, NavigationStart, NavigationEnd, NavigationError, ActivatedRoute, NavigationCancel } from '@angular/router';
import {
  WiDropDownList, CategoryList, WiDisciplineDropDownList, ExportComplianceList,
  ConfidentialitiesDropDownList, ApprovalRequirementIdList, GetClassifiersDropDownList,
  TagList, ExportAuthority, RestrictingProgram, GetRevionType
...
@Component({
  selector: 'app-top-eks-searchbar',
  templateUrl: './top-eks-searchbar.component.html',
  styleUrls: ['./top-eks-searchbar.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class TopEksSearchbarComponent implements OnInit {
  showDropDown = false;
  totalBadgeItems;
  todoItemsListData;
  userMail;
  ...
  constructor(
    ...
    private topDialogRef: MatDialogRef<TopEksSearchbarComponent>,
  ...
  onArrowClick() {
    this.getSearchSectionData(this.searchTerm);
    this.getEKSGloablSearchData();
    this.getEKSPWPlaySearchData();
  }
  ...
  closeTopModal(mytemplate) {
    console.log("mytemplate result", mytemplate);
