...
import { AddonsPopupTabDict, PopupExportedTypesDict } from './addons-popup-search.utils';
import { EksDetailItem } from './addons-popup-search.state';
@Component({
  selector: 'app-addons-popup-search',
  templateUrl: './addons-popup-search.component.html',
  styleUrls: ['./addons-popup-search.component.scss'],
  encapsulation: ViewEncapsulation.None,
  providers: [AddonsPopupSearchStore]
})
export class AddonsPopupSearchComponent implements OnInit {
  AddonsPopupTabDict = AddonsPopupTabDict;
  hiddenAddonsTabs: string[] = [];
  submitConfirmationResolve: () => Observable<boolean>;
  sltResultTabIdx = 0;
  ...
  excludedTypes$ = this.addonsPopupSearchStore.excludedTypes$;
  onLoading$ = this.addonsPopupSearchStore.onLoading$;
  sltEksResultItems$ = this.addonsPopupSearchStore.sltEksResultItems$;

  existingContentIds: any[] = [];

  @Input() set stepPopUp(value: string) {
    const excludedTypes = PopupExportedTypesDict[value] || [];
    this.addonsPopupSearchStore.patchState({ excludedTypes });
  }
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<AddonsPopupSearchComponent>,
    ...
    const excludedTypes = (data.popupType && PopupExportedTypesDict[data.popupType]) || [];
    this.addonsPopupSearchStore.patchState({ excludedTypes });
    ...
    this.existingContentIds = data.existingContentIds || [];
    this.disableObsoleteDocuments = !!data.disableObsoleteDocuments;
    this.hiddenAddonsTabs = data.hiddenAddonsTabs || [];
    this.submitConfirmationResolve = data.submitConfirmationResolve;
    console.log('existingContentIds: ', this.existingContentIds);
  }
  ...
  onSubmitSelectedItems(sltEksResultItems: EksDetailItem[]) {
    if (this.submitConfirmationResolve) {
      this.submitConfirmationResolve().subscribe(
        continuing => {
          continuing && this.dialogRef.close(sltEksResultItems);
        }
      )
    } else {
      this.dialogRef.close(sltEksResultItems);
    }
  }
