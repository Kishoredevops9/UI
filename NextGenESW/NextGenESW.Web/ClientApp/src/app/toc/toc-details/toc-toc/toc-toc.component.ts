...
export class TocTocComponent implements OnInit {
  ...
  expendSubject: Subject<boolean> = new Subject<boolean>();
  ...
  expendEventToChild() {
    this.eppOpt = (this.eppOpt)?false : true;
    this.expendSubject.next(  this.eppOpt );
  }
  ...
  addAddOns(type) {
    ...
    const currentContentIdList = compact(map(this.parentItem.children,'content.contentId'));
  ...
  componentParameters(content, parentId, isHeader) {
    ...
    createCompReq['version'] = this.version;
  ...
  getContentUpdateParent(element, child) {
    element &&  element.length &&  element.forEach(childL1 => {
      ...
      dialogRef.afterClosed().subscribe((result) => {
        if (result && result != 'No') {
          ...
          this.tocService.deleteTableOfContent(item.content.tableOfContentTocid).subscribe((data) => {
            ...
            this.isLoading = false;
            console.log("Delete item loading stop")
            this.emitEventToChild()
          });
        }
  ...
  loadTOCTOCData() {
    ...
    if (this.id) {
      this.contentCommonService.getTableOfContentData(this.contentNo, documentcontentType, documentStatusDetails, documentcontentId, documentversion, userEmail).subscribe(
        (res) => {
          this.tocTabData = res;
