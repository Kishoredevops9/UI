...
import { GuidedDraggingOptions } from './types';
 ...
 */
export class GuidedDraggingTool {
  ...
  private _guidelineSnapDistance = 8;
  private _isGuidelineEnabled = true;
  private _horizontalGuidelineColor = 'gray';
  private _verticalGuidelineColor = 'gray';
  private _centerGuidelineColor = 'gray';
  private _guidelineWidth = 1;
  private _searchDistance = 1000;
  private _isGuidelineSnapEnabled = true;

  /**
   * Constructs a GuidedDraggingTool and sets up the temporary guideline parts.
   */
  constructor(private diagram: go.Diagram, private options: GuidedDraggingOptions) {
    const partProperties = { layerName: 'Tool', isInDocumentBounds: false };
    const shapeProperties = { stroke: options.lineColor || 'gray', isGeometryPositioned: true };
   ...
   */
  get horizontalGuidelineColor(): string { return this.options.lineColor || this._horizontalGuidelineColor; }
  set horizontalGuidelineColor(val: string) {
    if (this._horizontalGuidelineColor !== val) {
      this._horizontalGuidelineColor = val;
      (this.guidelineHbottom.elements.first() as go.Shape).stroke = this.options.lineColor || this._horizontalGuidelineColor;
      (this.guidelineHtop.elements.first() as go.Shape).stroke = this.options.lineColor || this._horizontalGuidelineColor;
    }
  }
   ...
   * The default value is "gray".
   */
  get verticalGuidelineColor(): string { return this.options.lineColor || this._verticalGuidelineColor; }
  set verticalGuidelineColor(val: string) {
    if (this._verticalGuidelineColor !== val) {
      this._verticalGuidelineColor = val;
      (this.guidelineVleft.elements.first() as go.Shape).stroke = this.options.lineColor || this._verticalGuidelineColor;
      (this.guidelineVright.elements.first() as go.Shape).stroke = this.options.lineColor || this._verticalGuidelineColor;
    }
  }
   ...
   * The default value is "gray".
   */
  get centerGuidelineColor(): string { return this.options.lineColor || this._centerGuidelineColor; }
  set centerGuidelineColor(val: string) {
    if (this._centerGuidelineColor !== val) {
      this._centerGuidelineColor = val;
      (this.guidelineVcenter.elements.first() as go.Shape).stroke = this.options.lineColor || this._centerGuidelineColor;
      (this.guidelineHcenter.elements.first() as go.Shape).stroke = this.options.lineColor || this._centerGuidelineColor;
    }
  }
  ...
  public doDragOver(
    ...
    const draggingParts = copiedParts || draggedParts;
  ...
  protected isGuiding(part: go.Part, guidedpart: go.Part): boolean {
    return part instanceof go.Node &&
           part.key !== guidedpart.key &&
           guidedpart instanceof go.Node &&
           (this.options.blacklist || []).indexOf(part.category) === -1 &&
           (this.options.blacklist || []).indexOf(guidedpart.category) === -1 &&
